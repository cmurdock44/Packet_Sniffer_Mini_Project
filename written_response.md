...
startsniffer():
This funtction first prints a message that the packet capture has begun, then prompts the user to enter a number of packets they wish to capture. Next it uses the sniff function from the Scapy library to capture the number of packets prompted. For each packet that is captured, the process_packet() function is automatically called. After all ten packets are captured, the function prints a message marking the end of the function.
